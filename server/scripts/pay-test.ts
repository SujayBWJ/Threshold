/**
 * End-to-end x402 TestNet payment client for GET /api/test.
 * Uses official @x402/fetch + @x402/avm client APIs.
...
const testUrl = `${baseUrl.replace(/\/$/, "")}/api/test`;
...
async function main(): Promise<void> {
  ...
  console.log("=== Threshold x402 payment test ===");
  console.log(`Target: ${testUrl}`);

  // Step 0: health
  console.log("\n[1] GET / (health)");
  const health = await fetch(`${baseUrl.replace(/\/$/, "")}/`);
  ...
  console.log("\n[2] GET /api/test without payment");
  const unpaid = await fetch(testUrl, { method: "GET" });
  ...
  // Step 2: build paying client from payer mnemonic
  console.log("\n[3] Load payer signer from AVM_MNEMONIC");
  ...
  console.log("\n[4] Register ExactAvmScheme on Algorand TestNet");
  ...
  // Step 3–5: wrapFetchWithPayment handles 402 → sign → settle → retry
  console.log("\n[5] Request with x402 payment wrapper");
  console.log("    (on 402: construct/sign USDC payment, settle via facilitator, retry)");
  const fetchWithPayment = wrapFetchWithPayment(fetch, client);

  const response = await fetchWithPayment(testUrl, { method: "GET" });
  ...
  console.log("\n[6] Read settlement proof from response headers");
  ...
  if (paymentResponse && typeof paymentResponse === "object") {
    ...
    console.log(`    Settlement payload: ${JSON.stringify(paymentResponse)}`);
  } else {
