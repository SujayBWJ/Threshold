...
function updateSettlement(data) {
  ...
  if (transaction) {
    ...
    const networkPath = network?.includes("SGO1GKSzy") ? "testnet" : "mainnet";
...
function renderFlowResult(data) {
  ...
  if (scan) {
    ...
    agentResult.innerHTML = `
      ...
      <div class="agent-trace"><div class="trace-step"><span class="trace-index">01</span><div><strong>External lookup</strong><span>OSV-compatible mock security feed queried</span></div></div><div class="trace-step"><span class="trace-index">02</span><div><strong>Dependencies scanned</strong><span>${escapeHtml(String(data.response?.scan?.scannedDependencies ?? 0))}</span></div></div><div class="trace-step"><span class="trace-index">03</span><div><strong>Settlement</strong><span>${escapeHtml(data.transaction || "Facilitator response returned")}</span></div></div></div>
    `;
